...
権威DNSサーバーに同じ内容のゾーンを持たせるための仕組みとして、DNSの世界では@<kw>{ゾーン転送, zone transfer}と@<kw>{DNS NOTIFY}が定義されています。詳しい説明は省きますが、ゾーンに記載されている全ての情報を転送する仕組みと、プライマリ（ゾーン情報を管理する上でマスタとなるサーバー）からセカンダリ（マスタとなるサーバーからゾーン転送を受けて自身が持つ情報を更新するサーバー）にゾーンが更新されたことを通知する仕組みです。これらの仕組みを使わずに、手動でゾーンの情報を同期する方法もあります。
