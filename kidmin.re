...
グルーレコードは、本来は（ゾーンカットの先にあるため）委任先のゾーンが管理するべき情報であり、委任元の管理が及ばない（@<kw>{権威を持つ情報, authoritative data}ではない）情報です。しかし、前述のような理由でドメイン名空間のツリーをたどるために必要であるため、委任元のゾーンに記載されています。ですので、委任元の権威DNSサーバーに問い合わせても直接応答されるものではなく、委任を示す応答の一部としてのみ用いられます@<fn>{authority-of-ns-rrset}。フルリゾルバーも、委任を示す応答の一部として返ってきた情報は、反復検索のための一時的な情報として扱います。つまり、その情報だけで名前解決の結果としてクライアントに応答することはありません。
