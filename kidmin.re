...
ドメイン名空間全体をルートゾーンに記載し、@<tt>{www.sub.example.com}のような情報を直接登録するような運用は現実的ではありません。ルートゾーンの運用者が悲鳴を上げてしまうでしょう。
...
ゾーンの頂点以外の場所で@<tt>{NS}レコードが現れた場合、そのドメイン名から下のサブツリーは別のゾーンとして委任されていることを表しています。

委任する側（親側）と委任される側（子側）には、全く同じ内容のNSレコード（および、必要な場合にはグルーレコード）を記載します。それらが異なっている場合、@<kw>{不適切な委任, lame delegation}と呼ばれる状態となり、名前解決に支障が出る場合があります。
...
委任先である権威DNSサーバーのドメイン名が、その委任先ゾーンの中にある場合（これを@<kw>{内部名である権威DNSサーバー, in-domain name server}と呼びます）には、「缶切りは缶の中」状態になってしまいます。たとえば、@<tt>{example.com}ゾーンから@<tt>{sub.example.com}ゾーンを委任するにあたり、委任先の権威DNSサーバーのドメイン名を@<tt>{ns1.sub.example.com}にしたいとします。このとき、@<tt>{ns1.sub.example.com}というドメイン名は委任先である@<tt>{sub.example.com}ゾーンの中（もしくは、その委任先）に存在するため、NSレコードだけではそのIPアドレスを得ることができません。
...
グルーレコードは、本来は委任先のゾーンが管理するべき情報であり、（ゾーンカットの先にあるため）委任元の管理が及ばない情報です。しかし、前述のような理由でドメイン名空間のツリーをたどるために必要であるため、委任元のゾーンに記載されています。ですので、委任元の権威DNSサーバーに問い合わせても直接応答されるものではなく、@<kw>{委任を示す応答, referral response}の一部としてのみ用いられます。フルリゾルバーも、委任を示す応答の一部として返ってきた情報は、反復検索のための一時的な情報として扱います。つまり、その情報だけで名前解決の結果としてクライアントに応答することはありません。
...
一方、先に説明したとおり、グルーレコードは本来「委任先のゾーンが管理するべき情報」を、委任元に記載しています。このため、そのIPアドレスの変更にあたっては、自分の管理下にある委任先のゾーンの変更だけではなく、委任元への連絡と変更手続きが必要となってしまいます。
 ...
 * @<tt>{CNAME}レコードはエイリアス（別名）を定義する
 ** @<tt>{CNAME}の先（RDATAに記載されているドメイン名）が本名
 * なんとなく委任と似ているが、委任と違ってサブツリー全体ではなくそのドメイン名だけ本名を定義する
 ...
 *** 「別名」を定義するためのレコードなので、「本名」が複数ある状況はおかしい
 ** 例外あり: DNSSECでCNAMEの存在（と、それ以外のレコードの不在）を証明するための情報として、@<tt>{RRSIG}・@<tt>{NSEC}・@<tt>{NSEC3}の3種類のレコードだけは書いてもよいと変更された
 * @<tt>{CNAME}レコードの指す先が@<tt>{CNAME}レコードである場合もある
 ** CDNサービスでよく見かける
 ** RFCでは許容されているが、ループを作ってしまうことがある
 ** 無限ループを避けるために、多くの実装ではCNAMEをたどる回数に制限を設けている
 *** 主要な実装の例: Unboundは16段、BIND 9は32段（要出典）
 ** ループの問題や、名前解決の手間が増えることを考えると、多くとも2〜3段くらいにとどめておく（要出典）
 * DNAME……

== あとがき

本章では、ゾーンと委任という切り口でDNSについて説明しました。また、逆引きはDNSでどのように実現されているのか、@<tt>{CNAME}によるエイリアスの仕組みについて説明しました。
DNSにはこれ以外にも、キャッシュの仕組み（特に不在応答）、ワイルドカードの扱いといった厄介なものたちが存在します。本章では全く触れませんでしたが、これらはDNSにデータの出自の認証と完全性の保証を導入する@<kw>{DNSSEC, DNS Security Extensions}を、その落とし穴も含めて理解するために必要となります。また機会があれば、説明したいと思います。
