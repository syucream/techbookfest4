...
@<tt>{in-addr.arpa}および@<tt>{ip6.arpa}は、ルートゾーンからIPアドレスの割り振りを管理する組織などを経由して、IPアドレスブロックの管理者に委任されています。ただし、割り当てを受けている組織のポリシーにより、逆引きドメイン名を設定していなかったり、そもそも逆引きゾーンの委任がされていない場合もあります。

なお、IPv6アドレスの逆引きについて、昔は@<tt>{ip6.int}というゾーンも使われていました。しかし、@<tt>{ip6.arpa}に移行することとなり、2005年に発行されたRFC 4159により廃止されています。未だに@<tt>{ip6.int}と書いてある本があったら、窓から投げ捨ててよいでしょう。

===[column] どうして順序を逆にするの？

IPアドレスの割り当て階層構造と、DNSにおけるゾーンの委任の階層構造が異なるためです。
...
一方、ドメイン名はその逆で、左側ほど細かい単位の割り当て、右側ほど大きな単位での割り当てがなされています。たとえば@<tt>{jp}はルートゾーンからJPRSに委任され、その一部である@<tt>{tsukuba.ac.jp}は筑波大学が登録し、委任されています。

逆引き用ドメイン名で順序を逆にすることで、@<tt>{133.in-addr.arpa}ゾーンをJPNICに委任し、その一部である@<tt>{51.133.in-addr.arpa}ゾーンを筑波大学に委任する、といったように、IPアドレスの割り当て階層構造をゾーンの委任に揃えることができるのです。

なお、IPv4アドレスに関しては、@<tt>{/24}より細かい単位の割り当てをどのように委任するかという問題があります。本コラムでは説明しませんが、運用上はいくつかの手段が用意されており、委任元と調整して選択することとなります。
 ...
 ** 例外あり: DNSSECでCNAMEの存在（と、それ以外のレコードの不在）を証明するための情報として、@<tt>{RRSIG}・@<tt>{NSEC}・@<tt>{NSEC3}だけは書いてもよいと変更された
 * @<tt>{CNAME}レコードの指す先が@<tt>{CNAME}レコードである場合もある
 ** CDNサービスでよく見かける
 ** RFCでは許容されているが、ループを作ってしまうことがある
 ** 無限ループを避けるために、多くの実装ではCNAMEをたどる回数には制限を設けている
...
DNSにはこれ以外にも、キャッシュの仕組み（特に不在応答）、ワイルドカードの扱いといった厄介なものたちが存在します。これらは、本章では全く触れませんでしたが、DNSにデータの出自の認証と完全性の保証を導入する@<kw>{DNSSEC, DNS Security Extensions}を、その落とし穴も含めて理解するために必要となります。また機会があれば、説明したいと思います。
