 ...
 ** @<tt>{NS}レコードの内容はドメイン名で書き、IPアドレスは書けない
...
反復検索の過程で@<kw>{委任を示す応答, referral response}を受け取る（@<tt>{NS}レコードが返ってくる）と、委任先の権威DNSサーバーに改めて問い合わせることになります。しかし、@<tt>{NS}レコードの内容はドメイン名で記載されています。つまり、委任先に問い合わせるには、その委任先である権威DNSサーバーのドメイン名を名前解決する必要があるのです。権威DNSサーバーのIPアドレスであっても、ゾーンに含まれる他のドメイン名の情報と同じように@<tt>{A}レコードや@<tt>{AAAA}レコードで記載します。

委任先である権威DNSサーバーのドメイン名が、その委任先ゾーンの中にある場合（これを@<kw>{内部名である権威DNSサーバー, in-domain name server}と呼びます）には、「缶切りは缶の中」状態になってしまいます。たとえば、前節の例にある@<tt>{example.com}ゾーンから@<tt>{sub.example.com}ゾーンを委任するにあたり、委任先の権威DNSサーバーのドメイン名を@<tt>{ns1.sub.example.com}にしたいとします。このとき、@<tt>{ns1.sub.example.com}というドメイン名は委任先である@<tt>{sub.example.com}ゾーンの中（もしくは、その委任先）に存在するため、@<tt>{NS}レコードだけではそのIPアドレスを得ることができません。
...
これまで見てきたとおり、ドメイン名空間はたくさんのゾーンから構成されていて、ゾーンを別の権威DNSサーバーに委任することができます。ゾーンを切り出すことでドメイン名空間を分散して持つことができ、ある権威DNSサーバーはドメイン名空間の一部だけを持っていることになります。このようにして、パーティショニングが行われています。

委任の節で示した@<list>{DelegationExampleParentSide}の例にあるように、ゾーンを委任する先の指定、つまり@<tt>{NS}レコードは複数書くことができます。このとき、委任先として指定された権威DNSサーバーは、そのゾーンについて同じ情報を持つよう構成されています@<fn>{cdn-dns-load-balancing}。フルリゾルバーは、反復検索の過程で委任を見つけたとき、@<tt>{NS}レコードで指定された権威DNSサーバーから1つを選択して問い合わせを送ります。詳しくはフルリゾルバーの実装にもよりますが、問い合わせに対してしばらく応答がない場合にはタイムアウトしたものとして、他の権威DNSサーバーに再度問い合わせを行います。委任する先のどのサーバーも同じ情報を持っているので、どれか1台でも応答を得られることができれば、結果を得ることができるのです。このようにして、冗長性が実現されています。

//footnote[cdn-dns-load-balancing][CDNなどでロードバランシングを目的として、問い合わせ元のIPアドレスによって応答を変えることもあります]
...
本章では、ゾーンと委任という切り口でDNSについて説明しました。また、逆引きはDNSでどのように実現されているのか、他にも@<tt>{CNAME}によるエイリアスの仕組みについて説明しました。
