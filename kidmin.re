...
反復検索の過程で@<kw>{委任を示す応答, referral response}を受け取る（@<tt>{NS}レコードが返ってくる）と、委任先の権威DNSサーバーに改めて問い合わせることになります。しかし、@<tt>{NS}レコードの内容はドメイン名で記載されています。つまり、委任先に問い合わせるには、その委任先である権威DNSサーバーのドメイン名を名前解決する必要があるのです。
