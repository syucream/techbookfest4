...
@<tt>{CNAME}レコードの指す先は、存在するドメイン名でなければなりません。では、そのドメイン名に@<tt>{CNAME}レコードが記載されている場合、どうなるのでしょうか。RFCでは、このような複数段の@<tt>{CNAME}チェーンを許容しています。CDNサービスを利用しているWebサイトのドメイン名で、よく見かける構成です。

ただし、複数段の@<tt>{CNAME}チェーンを作る場合には注意が必要です。たとえば、@<tt>{a.example.com}の@<tt>{CNAME}レコードの指す先に@<tt>{b.example.com}を、逆に@<tt>{b.example.com}の@<tt>{CNAME}レコードの指す先に@<tt>{a.example.com}を指定すると、@<tt>{CNAME}チェーンのループができてしまいます。フルリゾルバーがこのドメイン名を名前解決しようとすると、無限ループに陥ってしまうでしょう。この対策として、多くの実装では@<tt>{CNAME}をたどる回数に制限を設けています。たとえば、Unboundでは8段までに制限されています。

複数段の@<tt>{CNAME}チェーンはRFCで許容されているものの、ループの問題や、フルリゾルバーでの名前解決の手間が増えることを考えると、多くとも2〜3段くらいにとどめておくのがよいでしょう。
