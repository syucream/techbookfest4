...
暗号通貨の基礎の話はあまり説明しません。それについてはこの本の「基礎から説明する暗号通貨」をご参照ください。


== IOTA とは

IOTA @<fn>{iota_official} は暗号通貨の一種であり、 IoT(Internet of Things) を支える基盤の一つを目指しています。
この IOTA はブロックチェインではなく Tangle という DAG(Directed Acyclic Graph, 非循環有向グラフ) の仕組みによってトランザクションを管理します。
...
MAM は IOTA のトランザクションのメッセージを暗号化し、閲覧者を絞ってそのメッセージを復号可能にする仕組みです。
...
イメージとしては下図のように、誰でも復号可能な public 、 key を共有された人のみが復号可能な restricted 、メッセージの root の値を持つ、送信した人のみが知りうる情報を持つ人のみが復号できる private があります。
...
前に触れた手数料無料の理由はこれで、マイニングに相当する仕事を自分でやることで手数料が発生しないようにしているのです。

なお他のトランザクションを承認する際に、 Bitcoin などでやるようにプルーフオブワークをすることになります。
...
しばらく待っても送金が完了しない場合は、取引履歴から送金トランザクションのリアタッチを行ってみましょう。
...
特に JavaScript 向けライブラリである `iota.lib.js`  @<fn>{iota.lib.js} の開発が進んでいるようです。
...
このコードでは以下のような具合で、任意のテキストを MAM の送信ポリシーである public, restricted, private のいずれかに基づいて送信することができます。
今回は送信後の MAM をすぐに fetch して読み出しています。

//cmd{
# 'BEER' を restricted mode で送信する
$ node mam.js BEER restricted
//}
...
MAM の送信で面白いのは restricted モードでの振る舞いでしょうか。
...
MAM の送信は IOTA を持っていない人でも行なうことができるので手軽に試せるかと思われます。
とはいえテスト用に MAM を送信したいのであれば、本番ネットワークではなく testnet を利用してみるのがおすすめです。
