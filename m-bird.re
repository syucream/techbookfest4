...
だいたいあっています。よく利用される用途としては、IPアドレスとドメイン名の結びつけに利用されることが多いです。しかし、他の用途もあります。具体的なものとしては、ドメイン名に任意のテキスト情報をドメイン名と結びつけることのできる、@<strong>{TXTレコード}の存在があります。

なお、TXTレコードは、メールの送信元なりすましを検知することができる仕組み@<strong>{SPF}(Sender Policy Framework)などに用いられています。
...
権威DNSサーバーは、頂点となる、ルートサーバーからドメイン名空間の一部の管理権限を@<strong>{委任}され、そのドメイン名空間の管理を行います。
...
このような特性より、フルリゾルバーは多くのユーザが共用すると、高速に結果を得ることができるため、ISPや会社組織単位などで運用が行われている場合が多いです。
...
=== 権威とキャッシュの同居とオープンリゾルバー問題
本章では、権威DNSサーバーとフルリゾルバーと、2つの役割のDNSサーバーが存在していることをご説明しました。
しかし、一部の運用ではフルリゾルバーと権威DNSサーバーを1台のホストで兼用している環境が存在しています。
...
また、@<strong>{DNSリフレクション攻撃}(DNSアンプ攻撃)と呼ばれる攻撃の加害者側として参加させられてしまう
問題があります。
DNSリフレクション攻撃は、@<strong>{オープンリゾルバー}と呼ばれる、インターネットのどこからでも問い合わせに
...
これは、権威DNSサーバーは、外部（インターネット側）からの問い合わせに応える必要があり、
そのような設定になっています。
権威DNSサーバーを同じサーバーで兼用していると、フルリゾルバー側もおのずとインターネット側からの
問い合わせに応えるような設定になってしまうためです。
...
しかし、実際には違います。ここまで説明したとおり、フルリゾルバーがキャッシュする期間は明示的に設定します。
