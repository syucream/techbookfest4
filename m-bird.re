...
また、フルリゾルバは名前解決した結果をキャッシュする機能もあります。
これにより、実行コストの高い「ドメイン名の階層構造を利用した名前解決」を何度も実行することを回避しています。人気なドメインであればキャッシュに乗りやすく、応答を高速に返すことが可能になります。
このような特性より、フルリゾルバは多くのユーザが共用して利用すると、高速に結果を得ることができるため、ISPや会社組織単位などで運用が行われている場合が多いです。

では、フルリゾルバはどのように処理を行い、フルリゾルバのユーザに応答を返しているのでしょうか。
